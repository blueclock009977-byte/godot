...
func test_hitbox_scale_front() -> void:
	...
	var dot: float = player_forward.dot(to_enemy)
	var angle: float = acos(clampf(dot, -1.0, 1.0))
	var scale: float = lerpf(1.0, 0.8, abs(sin(angle)))
...
func test_hitbox_scale_diagonal() -> void:
	...
	var player_pos := Vector3(0, 0, 0)
	var player_forward := Vector3(0, 0, -1)
	var enemy_pos: Vector3 = Vector3(10, 0, -10).normalized() * 10  # 45度
...
func test_hitbox_width_calculation() -> void:
	...
	# 正面（forward.x = 0）
	var width_front: float = lerpf(0.8, 0.1, 0.0)
	assert_almost_eq(width_front, 0.8, 0.01, "正面向きの幅は0.8")
	
	# 横（forward.x = 1）
	var width_side: float = lerpf(0.8, 0.1, 1.0)
