...
func test_point_budget_vanilla_cards():
	...
	for card in CardDatabase.card_pool:
		...
		var score := 8 * card.hp + 5 * card.atk + 4 * faces + int(synergy)
		var budget := 16 + 16 * card.mana_cost
		assert_true(absi(score - budget) <= 2, "%s score=%d budget=%d should be within ±2" % [card.card_name, score, budget])
