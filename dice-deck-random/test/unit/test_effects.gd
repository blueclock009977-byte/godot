...
func test_summon_effect_blue_018_uses_unified_log_format() -> void:
	...
	# blue_018 now uses _apply_aoe_status_effect which internally calls _make_effect_log
	assert_ne(script_text.find("_apply_aoe_status_effect(_get_all_enemies(is_player, context), StatusEffect.FROZEN, 1, result, card_name, \"cyan\", \"敵全体を凍結\")"), -1,
		"blue_018 should use shared AOE status effect helper")
...
func test_process_timing_event_dispatch_on_death_with_death_card_ui_alias() -> void:
	# 再現: ON_DEATH payload が death_card_ui だと死亡時効果が漏れる
	var death_card = _create_mock_card_ui("green_002")
	var result: Dictionary = EffectManager.process_timing_event(EffectManager.Timing.ON_DEATH, {
		"death_card_ui": death_card,
		"is_player": true,
		"context": _create_empty_context()
	})
	assert_eq(result.get("mana", 0), 1, "ON_DEATH should accept death_card_ui payload alias")
...
func test_process_timing_event_dispatch_on_defense_with_defend_card_ui_alias() -> void:
	# 再現: ON_DEFENSE payload が defend_card_ui だと defender解決できず防御効果が漏れる
	var defender = _create_mock_card_ui("yellow_004")
	var result: Dictionary = EffectManager.process_timing_event(EffectManager.Timing.ON_DEFENSE, {
		"defend_card_ui": defender,
		"damage": 5,
		"is_player": true,
		"context": _create_empty_context()
	})
	assert_eq(result.get("final_damage", -1), 4, "ON_DEFENSE should accept defend_card_ui payload alias")
...
func test_attack_mana_gain_effect_uses_shared_mana_helper() -> void:
	# 次の小さなリファクタ候補: 攻撃時のマナ獲得も共通helperへ統一
	var script_text := FileAccess.get_file_as_string("res://autoload/effect_manager.gd")
