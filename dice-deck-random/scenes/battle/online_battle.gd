...
func _opponent_summon(card_id: int, slot_idx: int) -> void:
	...
	_log("Opponent summons %s" % data_copy.card_name)
	_update_all_ui()
...
func _opponent_move(from_idx: int, to_idx: int) -> void:
	...
	_log("Opponent moves %s" % card.card_data.card_name)
	_update_all_ui()
...
func _do_dice_and_battle(dice_val: int) -> void:
	...
	await _resolve_attacks(turn_slots, def_slots, turn_is_me)
	if game_over:
		is_animating = false
		return
	await _resolve_attacks(def_slots, turn_slots, not turn_is_me)
	if game_over:
		is_animating = false
		return
	is_animating = false
