...
**この資料の手順を実行するときは、必ず `openclaw cron add` コマンドを使うこと。**
...
1. **isolated session**: 履歴汚染を防ぎ、クリーンな状態で実行
2. **タイムアウト管理**: 長時間タスクでも安全に実行
3. **sessions_send対応**: cronのisolated sessionからはsessions_sendが動作する
4. **復旧可能**: 失敗しても状態ファイルから再開可能

### メインセッションでの正しい動き

```markdown
## メインセッションで「チェーン式タスクを実行して」と言われたら

1. この資料（CRON_CHAIN_TECHNIQUE.md）を読む
2. CRON_BASICS.md も確認
3. **cronを設定する**（直接実行しない！）
4. 「cronを設定しました。実行を開始します」と報告
5. 以降の作業はcronが自律的に進める
```
...
3. **完了報告**（作業終了時）
   sessions_sendツールで Discord channel:チャンネルID に報告：
   「✅ Phase N 完了: [やったこと] → 次cron: [設定した/しなかった(理由)]」
...
- 変更内容
- 指標差分
- **次cron設定: あり（Phase N+1）/ なし（理由: 収束/Usage不足/エラー等）**
```
